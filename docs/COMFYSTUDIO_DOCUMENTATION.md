...
**Fixed with a scoped, not blunt, solution:** rather than making the whole system `site-packages` writable (which would let a bad install overwrite an already-vetted baked-in package like `torch` or `numpy`), redirected Manager's installs to Python's existing per-user site-packages location instead — `/home/comfyuser/.local/lib/python3.11/site-packages`, a path `comfyuser` already owns and Python already auto-discovers via `site.ENABLE_USER_SITE`, needing no `PYTHONPATH` changes. Since Manager's own source never passes `--user` and can't be safely patched (it overwrites itself on its next self-update), the redirect had to work via `uv`'s own config-file discovery: new `docker/uv.toml` with `[pip] target = ".../comfyuser/.local/.../site-packages"`, activated globally via `ENV UV_CONFIG_FILE=...` in the Dockerfile. Tested several alternatives first before landing here — a `PIP_USER=1` environment variable does **not** work (`uv` doesn't honor that pip convention; confirmed via a real failed install with the env var set), and `uv`'s own `uv.toml` schema has no `user` field at all (confirmed via its own error message listing every valid field) — `target`/`prefix` are the only viable levers.

Verified thoroughly, end-to-end, not just applied: built the image locally, confirmed all 5 existing custom nodes still import cleanly, then ran the *exact* dependency that originally failed (`deepdiff`, pulling in `orderly-set`) via a real `uv pip install` with zero special flags — it now installs cleanly into `comfyuser`'s user-site and imports successfully. Separately confirmed system `site-packages` is still genuinely protected (a `touch` inside it fails with `Permission denied`, unchanged from before).

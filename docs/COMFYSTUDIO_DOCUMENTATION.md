...
deploy:
  resources:
    limits:
      ...
      memory: 48G
```
`CHOWN`/`SETUID`/`SETGID` are the exact three capabilities the entrypoint's privilege-drop sequence needs (the `chown` on `/app/ComfyUI/user`, then `gosu` dropping root → `comfyuser`) — everything else a default container gets (raw networking, kernel module loading, etc.) is now unavailable to any code running inside, custom nodes included. The CPU/RAM limits exist because multiple projects run side-by-side on one host — without a cap, one runaway workflow could starve every other project's container.
...
**Memory limit revised 32G → 48G the same day**, after real testing revealed the original number was tighter than it looked: the sandbox sat at 27.25GiB/32GiB (85%) after a round of normal use. Investigated before just bumping the number — confirmed via `docker info` that Docker Desktop for Windows runs inside a WSL2 VM with its own memory ceiling, separate from physical RAM (this machine has 125.6GB physical RAM, but no `.wslconfig` exists, so WSL2 defaults to roughly half of that, ~61.6GB — confirmed, not assumed). GPU VRAM is completely unaffected by any of this either way — confirmed via `nvidia-smi` from inside the container showing the full card, since our `memory:` limit is a Linux cgroup (system RAM) limit, and neither Docker Compose nor the NVIDIA container runtime caps VRAM at all (outside of enterprise MIG-partitioned datacenter cards, not applicable here). Settled on 48G — leaves ~13.6GB of the ~61.6GB WSL2 budget for the OS/Docker daemon/other containers, without raising the WSL2 ceiling itself (a bigger, more disruptive change via `.wslconfig` + a restart, not pursued since there's no concurrent-multi-project need for it yet). Re-verified clean after the change: a freshly recreated container sits at under 2% of the new cap, confirming most of that original 85% reading was reclaimable OS page cache from reading large model files, not real memory pressure.

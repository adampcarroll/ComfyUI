...
### Resolved (2026-09-09) — ComfyUI-Manager couldn't install new custom nodes at all (real user-reported bug, not theoretical)
User tried installing `ComfyUI-Crystools` (a resource-monitoring node) via Manager's UI and got a real failure: `Failed to create directory /usr/local/lib/python3.11/site-packages/orderly_set: Permission denied`. Root cause, same family as the CRLF/dubious-ownership bugs from earlier this week: Manager's install flow runs `python3.11 -m uv pip install -U <packages>` (confirmed by reading Manager's own source, `manager_util.py`'s `get_pip_cmd`/`make_pip_cmd`) with no `--user`/`--target` flag, against the root-owned system `site-packages` — any node needing a Python dependency not already baked into `requirements.lock` fails outright, every time, in internet-enabled mode.

Discussed the fix as an architecture question before building anything, since the direct fix (make system `site-packages` writable) opens a real question about custom-node trust. Conclusion: a malicious custom node is already arbitrary Python running as `comfyuser`, with full access to that project's own mounted folders and the network if enabled — that risk exists the moment any unvetted node's code runs at all, regardless of site-packages permissions. What actually matters is containing the blast radius: each project is its own container with its own isolated Docker network and its own copies of `custom_nodes`/`models`/`input`/`output` (verified, not assumed — a compromised container cannot see an unmounted path, full stop, since Docker containers have their own filesystem namespace; only explicitly bind-mounted host paths bridge across). LucidLink's own snapshot/versioning feature (once this pipeline moves there) is a legitimate rollback for file damage — though it doesn't undo something already exfiltrated over the network before a rollback. (ComfyUI-Manager's own in-app "Snapshots" feature is a different thing entirely and is NOT a real rollback — see the "Snapshots" note below.)

**First attempt (tried, then reverted the same day): a scoped user-site redirect.** Rather than making the whole system `site-packages` writable (which would let a bad install overwrite an already-vetted baked-in package like `torch` or `numpy`), redirected Manager's installs to Python's existing per-user site-packages location instead — `/home/comfyuser/.local/lib/python3.11/site-packages`, a path `comfyuser` already owns and Python already auto-discovers via `site.ENABLE_USER_SITE`, needing no `PYTHONPATH` changes. Since Manager's own source never passes `--user` and can't be safely patched (it overwrites itself on its next self-update), the redirect had to work via `uv`'s own config-file discovery: `docker/uv.toml` with `[pip] target = ".../comfyuser/.local/.../site-packages"`, activated globally via `ENV UV_CONFIG_FILE=...`. Ruled out two other levers first, empirically: a `PIP_USER=1` environment variable does **not** work (`uv` doesn't honor that pip convention), and `uv.toml`'s own schema has no `user` field at all (confirmed via its own error message listing every valid field) — `target`/`prefix` were the only viable options.

**This made things WORSE, not better — caught via real usage, not theory.** The user actually installed `ComfyUI-Crystools` through this fix and hit two new problems: (1) Manager itself started logging `[ComfyUI-Manager] PyTorch is not installed` on every startup, and (2) `numpy` and `Pillow` turned up duplicated between system `site-packages` (our pinned `2.4.5`/`12.2.0`) and the new user-site (unpinned `2.4.6`/`12.3.0`, pulled in as transitive dependencies of the new node). Root cause: `uv pip install --target` doesn't check what the surrounding environment already satisfies — it treats the target directory as a self-contained install and pulls in its own copies of things, even when a compatible version already exists in system `site-packages`. Since Python's user-site takes precedence over system site-packages on import, those unvetted duplicate versions **silently became the ones actually running**, for the entire ComfyUI process — not just the new node. This is the opposite of the isolation the fix was meant to provide, and worse than a plain in-place upgrade would have been.

**Reverted to the direct fix instead: chown system `site-packages` to `comfyuser` at build time**, right alongside the existing `chown -R comfyuser /app`. A normal `uv pip install -U` against a writable `site-packages` correctly recognizes an already-installed package and upgrades it *in place* — one canonical version on disk, no shadow copy, matching the trade-off every other ComfyUI install in the world already accepts (a single shared Python environment where a new node's dependencies can bump a shared package). Verified end-to-end against the real regression: built the image locally, confirmed all 5 baseline custom nodes still import, confirmed a real `uv pip install deepdiff` (the exact dependency Crystools needed) installs directly into system `site-packages` with **zero shadow copy anywhere**, confirmed `torch`/`numpy`/`Pillow` versions stayed exactly at their pinned `requirements.lock` values, and confirmed Manager's own `uv pip list` correctly reports `torch` as installed again.

One accepted, understood limitation, left as-is deliberately: a package's optional console-script entry point (e.g. `deepdiff`'s `deep` CLI) still can't install, since that goes to `/usr/local/bin` — which also holds `gosu` and `python3.11` themselves. That directory is deliberately **not** made writable; doing so would let a bad install overwrite the actual privilege-drop mechanism and interpreter, a far more severe risk than a missing CLI convenience. Custom nodes consume their dependencies as importable libraries, not installed CLI commands, so this doesn't affect real node functionality.

**Also fixed in the same pass, found while reasoning through project isolation:** `Shared_Assets/extra_model_paths.yaml` was the one mount in the whole architecture that was shared and writable across every project's container — everything else (`Shared_Assets` itself, each project's own folders) was already correctly scoped, but this one file wasn't `:ro`. Fixed in both scripts' compose generation.
